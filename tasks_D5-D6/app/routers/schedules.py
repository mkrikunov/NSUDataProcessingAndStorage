...
from app.models import ScheduleIn, ScheduleOut
from app.routers.utils import load_query, parse_schedule
...
@router.get("/airport/{airport_code}/inbound_schedule", response_model=List[ScheduleIn], responses={
...
async def get_inbound_schedule(airport_code: str):
    try:
        query = load_query(get_inbound_schedule_for_an_airport_file)
        ...
        return [parse_schedule(row, flag="is_inbound") for row in results]
    except Exception:
        raise HTTPException(status_code=500, detail="Internal server error.")


@router.get("/airport/{airport_code}/outbound_schedule", response_model=List[ScheduleOut], responses={
...
async def get_outbound_schedule(airport_code: str):
    try:
        query = load_query(get_outbound_schedule_for_an_airport_file)
        results = await database.fetch_all(query, values={"airport_code": airport_code})
        if not results:
            raise HTTPException(status_code=404, detail="No outbound schedule found for the airport.")
        return [parse_schedule(row, flag="is_outbound") for row in results]
